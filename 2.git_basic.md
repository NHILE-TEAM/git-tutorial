...
```bash
git config --global user.name "NhiLeTeam"
git config --global user.email nhileteam@gmail.com
```
...
2. Mở terminal lên paste đường link đó kèm với câu lệnh `git clone`, ví dụ:
   ...
   ```bash
   git clone https://github.com/NHILE-TEAM/git-tutorial.git
   ```
   ...
   ```bash
   git clone https://username:password@github.com/git-tutorial.git
   ```
...
```bash
git clone git@github.com:NHILE-TEAM/git-tutorial.git
```
...
```bash
Your identification has been saved in /c/Users/user/.ssh/id_nhileteam
Your public key has been saved in /c/Users/user/.ssh/id_nhileteam.pub
```

Để đọc nội dung public SSH key thì bạn chỉ có khá nhiều cách, bạn dùng cách nào dưới đây cũng được. Ví dụ file public key của mình bên trên là `id_nhileteam.pub`

- Copy đường dẫn này `c:/Users/user/.ssh/id_nhileteam.pub` bỏ lên Chrome thì nó sẽ ra nội dung của public key
- Dùng git bash gõ `cat /c/Users/user/.ssh/id_nhileteam.pub`
...
#React-Awesome nhileteam
Host github-nhileteam.com
HostName github.com
User git
IdentityFile ~/.ssh/id_rsa_nhileteam
```

Ở trên thì mình có dùng 2 tài khoản Github, cái đầu tiên dùng với private key là `id_rsa` với Host là `github.com`, cái thứ hai là `id_rsa_nhileteam` với Host là `github-nhileteam.com`.
...
```bash
git clone git@github-nhileteam.com:dreact04072022/git-can-ban.git
```

Tương tự với những câu lệnh như `git remote add origin git@github-nhileteam.com:dreact04072022/git-can-ban.git`
